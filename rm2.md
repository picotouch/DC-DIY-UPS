...
- **Lead-Acid BatterySpecifications:**
...
Standard UPS battery, widely available
...
While the modules work without modifications, the following changes can made:
...
Detailed pictures are at the end of this document (LTC3780 modification).
...
| **Vout** | **R-down (Original)** | **R-up (Original)** | **R-down (Option 1)** | **R-up (Option 1)** | **R-down (Option 2)** | **R-up (Option 2)** |
| --- | --- | --- | --- | --- | --- | --- |
...
**Material:** 1.5mm galvanized sheet metal **Design:** Two identical U-shaped sections

**Width:** 182mm (matches Dell Optiplex Micro)

**Features:** Air circulation openings, mounting holes for PCB brackets

- **3D Printed ComponentsFront/Rear Panels:**
...
**GX16 pin assignments (this avoid miscmatch connection):**
...
**Status:** Require external Crowbar circuit (future), so set &lt;bP:0&gt; (disabled)
...
By implementing adequate hysteresis (0.7-1.2V) between ON and OFF thresholds, combined with prioritybased sequential shutdown, we can:
...
| NAS | 3rd/1rd | 13.00V | 12.30V | 0.70V |
...
- **Configure and solder Rcs resistor** based on battery capacity
- **Install battery charger module** on DIYUPS3 PCB
- **Modify DC/DC converters** (optional but recommended)
- **Set output voltages** via fixed resistors or potentiometers
- **Mount components** in enclosure using 3D printed holders
- **Connect wiring:**
...
- **Configure software** via USB serial connection
- **Test voltage thresholds** with actual load

**Note:**

- Sytem will turn ON immediately, regardless of which power source is connected, external or battery
- Battery need to be connected before external power supply
...
- use switch to enter device name belong each DC/DC
...
LTC3780 is DC/DC buck-boost converter IC

Pre-assembled modules featuring the popular LTC3780 chip (Analog Devices, [LTC3780 Rev G](https://www.analog.com/media/en/technical-documentation/data-sheets/ltc3780.pdf)) were used as the DC/DC converter.
...
\*Red line accross resistor R007 is wire connected instead resistor

- **removal of the UVLO function (under-voltage lockout):** This function prevents excessive battery discharge when running on battery. Since the same function is implemented in the ATtiny microcontroller (and is user-programmable), the module UVLO is not required.
...
\*all in yellow is UVLO

**Voltage adjustment (CV):** The output voltage is controlled by a multi-turn potentiometer, which can be replaced with a fixed resistor. The potentiometer can cause voltage fluctuations due to its high resistance (500 kΩ) being sensitive to touch, or due to contact loss in low-quality units/parts. Replacing it with a fixed resistor solves both issues. Use a 1206 SMD resistor for R-up and an 0805 for R-down.

The formula for calculating resistors for different output voltages is:

Vout=0,8V\*(1+R<sub>up</sub>/R<sub>down</sub>)

Values for various voltages are listed in the table:

|     | **Original R-down** |     | **Changed R-down** |     | **Changed R-down** |     |
| --- | --- |     | --- |     | --- |     | --- | --- | --- |
| **Vout** | **R-down** | **R-up** | **R-down** | **R-up** | **R-down** | **R-up** |
| **5** | **14K** | **75K** | **10K** | **53,6K** | **6,8K** | **35,7K** |
| **9** | **14K** | **123K** | **10K** | **102K** | **6,8K** | **98,8K** |
| **12** | **14K** | **196K** | **10K** | **140K** | **6,8K** | **95,3K** |
| **15** | **14K** | **249K** | **10K** | **178K** | **6,8K** | **121K** |
| **19** | **14K** | **328K** | **10K** | **234K** | **6,8K** | **158K** |

\*Do not use R-down lower than 6.8K.

These changes (CC and CV) are not necessary for normal working.

\*green is R_down

\*yellow 500K is potentiometer(remove), R_up is SMD to solder

**Complete picture:**

CC-Vout-UVLO schematic from internet:

CC (VR2), Vout(VR3)

UVLO(VR1)

**Battery charger module (CN3767 IC):**

**3D print parts:**

- DIYUPS3 main PCB, 2pcs
- LTC3780 DC/DC, 2 pcs (for 3 modules)
- Legs below LTC3780, 2 pcs
- Front side for enclosure and battery, 2pcs; back side, 2pcs
